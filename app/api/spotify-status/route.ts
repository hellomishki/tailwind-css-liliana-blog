...
async function fetchSpotifyData(accessToken: string) {
  ...
  if (currentTrackResponse.status === 200) {
    ...
    const recentTracksResponse = await fetch('https://api.spotify.com/v1/me/player/recently-played?limit=1', {
      headers: { 'Authorization': `Bearer ${accessToken}` }
    })
  ...
  if (sse === 'true') {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        while (true) {
          try {
            if (counter % 2 === 0) {  // Send ping every minute
            ...
            controller.enqueue(
              encoder.encode(`data: ${JSON.stringify({ error: 'Error fetching Spotify data' })}\n\n`)
            )
          }
